...
(function() {
  var ExportMap, INDEX_PATH_REGEX, NON_ROOT_DIRECTORY_IGNORE_PATTERN_REGEX, PARENT_DIRECTORY_NAME_REGEX, PATH_SEP_IN_REGEX, addDefaultBlacklist, appendToCacheKey, configFileBasenames, createDirectoryCache, defaultBlacklist, directoryCaches, ensureArray, ensureLeadingDot, ffilter, findKnownImport, findKnownImportInDirectory, first, fmapValues, fs, getAddImportFix, getExportMap, getExtensions, getIsNonRootDirectoryPattern, getParentDirectoryName, getRemoveImportFix, isArray, isFresh, isString, knownImportExists, knownImportsCache, last, loadConfigFile, loadKnownImports, mergeKnownImportsField, mergeWith, normalizeKnownImportValue, normalizeKnownImports, normalizePath, normalizePathUnix, pathModule, picomatch, pkgDir, shouldIncludeBlankLineBeforeLocalImports, sortBy, startsWith, stripIndex, takeWhile, toOsPathSeparators, toUnixPathSeparators, updateDirectoryCache,
    indexOf = [].indexOf;
  ...
  getParentDirectoryName = function(prefixRelativePath) {
    var match;
  ...
  createDirectoryCache = function({
    ...
    scanDir = function(dir) {
      var dirName, exports, ext, file, fullPath, i, len, name, namedExport, prefixRelativePath, ref, ref1, relativePathWithExtension, relativePathWithExtensionUnix, results;
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        relativePathWithExtension = fullPath.startsWith(`${directory}${pathModule.sep}`) ? fullPath.slice(`${directory}${pathModule.sep}`.length) : fullPath.startsWith(`${directory}`) ? fullPath.slice(directory.length) : fullPath;
        relativePathWithExtensionUnix = toUnixPathSeparators(relativePathWithExtension);
        if (fs.statSync(fullPath).isDirectory()) {
          if (ignoreMatcherNoBasename(relativePathWithExtensionUnix)) {
            continue;
          }
          if (ignoreMatcherContains(relativePathWithExtensionUnix)) {
            continue;
          }
          if (recursive) {
          ...
          if (ignoreMatcher(relativePathWithExtensionUnix)) {
            continue;
          }
          ({
            dir: dirName,
          ...
          if (name === 'index' && prefixRelativePath) {
            name = (ref1 = getParentDirectoryName(prefixRelativePath)) != null ? ref1 : name;
            fullPath = fullPath.replace(RegExp(`${PATH_SEP_IN_REGEX}index${ext}$`), '');
          }
          if (indexOf.call(allowed, 'filename') >= 0) {
            (function() {              // eslint-disable-next-line coffee/no-loop-func
              if (indexOf.call(extensions, ext) < 0) {
                return;
              }
              if (exports != null) {
                if (!exports.namespace.has('default')) {
                  return;
                }
              }
              return appendToCacheKey(cache, settings['known-imports/case-insensitive-whitelist-filename'] ? name.toLowerCase() : name, {
                prefixRelativePath,
                fullPath,
                type: 'filename'
              });
          ...
          if (indexOf.call(allowed, 'named') >= 0) {
            if (exports != null) {
              results.push((function() {
                var ref2, results1;
                ref2 = exports.namespace.keys();
                results1 = [];
                for (namedExport of ref2) {
  ...
  PATH_SEP_IN_REGEX = pathModule.sep === '/' ? '/' : '\\\\';
  ...
  normalizePath = function(path) {
    ...
    if (RegExp(`${PATH_SEP_IN_REGEX}$`).test(path)) {
      return path;
    }
    return `${path}${pathModule.sep}`;
  };

  normalizePathUnix = function(path) {
    if (!path) {
      return path;
    }
    if (/\/$/.test(path)) {
  ...
  toOsPathSeparators = function(path) {
    return path.replace(/\//g, pathModule.sep);
  };

  toUnixPathSeparators = function(path) {
  ...
  findKnownImportInDirectory = function({directory, allowed = ['filename'], prefix = '', recursive = true, extensions, name, settings, context, projectRootDir, ignore}) {
    var ambiguousMatches, closestFirst, directoryCache, extension, filename, found, foundCaseInsensitive, foundExact, getDotsPrefixLength, getRelativePath, hasAmbiguousMatches, haveSameSharedParentDirectory, importPath, prefixRelativePath, ref, ref1, relativePath, relativePathWithoutExtension, shortestDotsPrefixLength, type;
    ...
    getDotsPrefixLength = function(match) {
      var _, dotsPrefix;
      [_, dotsPrefix] = RegExp(`^((?:\\.\\.${PATH_SEP_IN_REGEX // eslint-disable-line coffee/no-unused-vars
    ...
    found = hasAmbiguousMatches ? (ambiguousMatches = foundExact.length ? foundExact : foundCaseInsensitive, closestFirst = sortBy(getDotsPrefixLength)(ambiguousMatches), shortestDotsPrefixLength = getDotsPrefixLength(closestFirst[0]), haveSameSharedParentDirectory = takeWhile(function(match) {
      ...
      return getRelativePath(match).replace(RegExp(`${PATH_SEP_IN_REGEX}[^${PATH_SEP_IN_REGEX}]+$`), '').length;
    })(haveSameSharedParentDirectory))) : [...foundExact, ...foundCaseInsensitive][0];
    if (!found) {
      return null;
    }
    ({prefixRelativePath, type} = found);
    importPath = settings['known-imports/relative-paths'] ? (relativePath = getRelativePath(found), extension = pathModule.extname(relativePath), relativePathWithoutExtension = indexOf.call(extensions, extension) >= 0 ? pathModule.join(pathModule.dirname(relativePath), pathModule.basename(relativePath, extension)) : relativePath, ensureLeadingDot(toUnixPathSeparators(relativePathWithoutExtension))) : `${prefix}${prefixRelativePath}`;
    return {
      module: importPath,
