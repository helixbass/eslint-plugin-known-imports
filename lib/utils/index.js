...
(function() {
  var ExportMap, addDefaultBlacklist, configFileBasenames, createDirectoryCache, defaultBlacklist, directoryCaches, ensureLeadingDot, ffilter, findKnownImport, findKnownImportInDirectory, fmapValues, fs, getAddImportFix, getExportMap, getExtensions, isFresh, isString, knownImportExists, knownImportsCache, last, loadConfigFile, loadKnownImports, mergeKnownImportsField, mergeWith, normalizeKnownImportValue, normalizeKnownImports, normalizePath, pathModule, pkgDir, shouldIncludeBlankLineBeforeLocalImports, startsWith, updateDirectoryCache,
    indexOf = [].indexOf;
  ...
  findKnownImportInDirectory = function({directory, allowed = ['filename'], prefix = '', recursive = true, extensions, name, settings, context, projectRootDir}) {
    var directoryCache, extension, filename, found, fullPath, importPath, prefixRelativePath, relativePath, type;
    if (projectRootDir != null) {
    ...
    importPath = settings['known-imports/relative-paths'] ? (relativePath = pathModule.relative(pathModule.dirname(filename), fullPath), extension = pathModule.extname(relativePath), ensureLeadingDot(indexOf.call(extensions, extension) >= 0 ? pathModule.join(pathModule.dirname(relativePath), pathModule.basename(relativePath, extension)) : relativePath)) : `${prefix}${prefixRelativePath}`;
    return {
      module: importPath,
