...
(function() {
  ...
  module.exports = util.createRule({
    ...
    defaultOptions: [{}],
    create: function(context, [firstOption]) {
      ...
      options = (function() {
        var _options, ref1, ref2, ref3, ref4, ref5;
        _options = {
          vars: 'all',
          args: 'after-used',
        ...
        if (firstOption) {
          if (typeof firstOption === 'string') {
            ...
            if (firstOption.destructuredArrayIgnorePattern) {
              _options.destructuredArrayIgnorePattern = new RegExp(firstOption.destructuredArrayIgnorePattern, 'u');
            }
          }
        }
        return _options;
      ...
      collectUnusedVariables = function() {
         ...
         */
        var def, hasRestSibling, hasRestSpreadSibling, isAfterLastUsedArg, ref1, ref2, ref3, ref4, ref5, refUsedInArrayPatterns, unusedVariablesOriginal, unusedVariablesReturn, variable;
        hasRestSibling = function(node) {
          var ref1;
          return node.type === AST_NODE_TYPES.Property && ((ref1 = node.parent) != null ? ref1.type : void 0) === AST_NODE_TYPES.ObjectPattern && node.parent.properties[node.parent.properties.length - 1].type === AST_NODE_TYPES.RestElement;
        };
        hasRestSpreadSibling = function(variable) {
          var hasRestSiblingDefinition, hasRestSiblingReference;
          if (options.ignoreRestSiblings) {
            hasRestSiblingDefinition = variable.defs.some(function(def) {
              return hasRestSibling(def.name.parent);
            });
            hasRestSiblingReference = variable.references.some(function(ref) {
              return hasRestSibling(ref.identifier.parent);
            });
        ...
        for (variable of unusedVariablesOriginal) {
          ...
          refUsedInArrayPatterns = variable.references.some(function(ref) {
            var ref1;
            return ((ref1 = ref.identifier.parent) != null ? ref1.type : void 0) === AST_NODE_TYPES.ArrayPattern;
          });
          if ((((ref1 = def.name.parent) != null ? ref1.type : void 0) === AST_NODE_TYPES.ArrayPattern || refUsedInArrayPatterns) && 'name' in def.name && ((ref2 = options.destructuredArrayIgnorePattern) != null ? ref2.test(def.name.name) : void 0)) {
            continue;
          }
          ...
          if (def.type === TSESLint.Scope.DefinitionType.CatchClause) {
            ...
            if ('name' in def.name && ((ref3 = options.caughtErrorsIgnorePattern) != null ? ref3.test(def.name.name) : void 0)) {
          ...
          if (def.type === TSESLint.Scope.DefinitionType.Parameter) {
            ...
            if ('name' in def.name && ((ref4 = options.argsIgnorePattern) != null ? ref4.test(def.name.name) : void 0)) {
              // skip ignored parameters
              continue;
            }
            ...
            if ('name' in def.name && ((ref5 = options.varsIgnorePattern) != null ? ref5.test(def.name.name) : void 0)) {
      ...
      return {
        ...
        'TSModuleDeclaration > TSModuleDeclaration': function(node) {
          var scope, superVar;
        ...
        'Program:exit': function(programNode) {
           ...
           */
          var directiveComment, getAssignedMessageData, getDefinedMessageData, i, len, unusedVar, unusedVars, writeReferences;
          ...
          getAssignedMessageData = function(unusedVar) {
            var additional, def, ref1;
            def = unusedVar.defs[0];
            additional = '';
            if (options.destructuredArrayIgnorePattern && (def != null ? (ref1 = def.name.parent) != null ? ref1.type : void 0 : void 0) === AST_NODE_TYPES.ArrayPattern) {
          ...
          for (i = 0, len = unusedVars.length; i < len; i++) {
            ...
            if (unusedVar.defs.length > 0) {
              writeReferences = unusedVar.references.filter(function(ref) {
                return ref.isWrite() && ref.from.variableScope === unusedVar.scope.variableScope;
              });
              context.report({
