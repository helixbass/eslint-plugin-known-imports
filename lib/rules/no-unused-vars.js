...
const {loadKnownImports} = require('../utils')
...
module.exports = {
  ...
  create(context) {
    ...
    if (firstOption) {
      if (typeof firstOption === 'string') {
        ...
        config.onlyRemoveKnownImports =
          firstOption.onlyRemoveKnownImports != null
            ? firstOption.onlyRemoveKnownImports
            : config.onlyRemoveKnownImports
    ...
    const knownImports = config.onlyRemoveKnownImports && loadKnownImports()
    ...
    function getFix({unusedVar, context}) {
      const def = unusedVar.defs[0]
      if (def.type !== 'ImportBinding') return null
      if (config.onlyRemoveKnownImports && !knownImports[unusedVar.name])
        return null
      ...
      return fixer => {
        ...
        const importSpecifier = def.node
        const nextToken = sourceCode.getTokenAfter(importSpecifier)
        if (nextToken.value === ',') {
          const followingToken = sourceCode.getTokenAfter(nextToken)
          return fixer.removeRange([
            importSpecifier.range[0],
            followingToken.range[0],
          ])
        }
        const precedingToken = sourceCode.getTokenBefore(importSpecifier)
        if (precedingToken.value === ',') {
          const beforeToken = sourceCode.getTokenBefore(precedingToken)
          return fixer.removeRange([
            beforeToken.range[1],
            importSpecifier.range[1],
          ])
        }
        const isOnlyNamedImport =
          importSpecifier.type === 'ImportSpecifier' &&
          importDeclaration.specifiers.filter(
            ({type}) => type === 'ImportSpecifier'
          ).length === 1
        if (isOnlyNamedImport) {
          const openingBrace = sourceCode.getTokenBefore(importSpecifier, {
            filter: ({value}) => value === '{',
          })
          const closingBrace = sourceCode.getTokenAfter(importSpecifier, {
            filter: ({value}) => value === '}',
          })
          const precedingComma = sourceCode.getTokenBefore(openingBrace)
          // assert(precedingComma.value === ',')
          const beforeToken = sourceCode.getTokenBefore(precedingComma)
          return fixer.removeRange([
            beforeToken.range[1],
